...
<nav {{ $attributes->merge(['class' => 'flex text-sm text-gray-500 dark:text-gray-400 pt-6 mb-4']) }} aria-label="Breadcrumb">
    <ol class="inline-flex items-center flex-wrap gap-y-1" itemscope itemtype="https://schema.org/BreadcrumbList">
        <li class="inline-flex items-center" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
            <a href="{{ locale_path('/') }}" wire:navigate class="inline-flex items-center hover:text-blue-600 dark:hover:text-blue-400 transition-colors" itemprop="item">
        ...
        @foreach($items as $label => $url)
            <li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
                <div class="flex items-center">
                    ...
                    @if(!$loop->last)
                        <a href="{{ $url }}" wire:navigate class="text-sm font-medium hover:text-blue-600 dark:hover:text-blue-400 transition-colors" itemprop="item">
